...
class CacheTests(autograde.FileTests):
    ...
    TestClass = autograde.FileRefTest
    ...
    def get_tests(self):
        ...
        for ref_name in fnames:
            ...
            tracefile = os.path.join(self.data_dir, f'trace{trace}.txt')
            ...
            yield super().test(
                self.prog, csize, assoc, self.repl, bsize, tracefile,
                ref_file = ref)

assignment = autograde.Project('cachesim',
    CacheTests(id='fifo', weight=2, ref_prefix='test.'),
    CacheTests(id='lru', weight=0.3, category=autograde.EXTRA, ref_prefix='test.'),
    user_class=None,
)
